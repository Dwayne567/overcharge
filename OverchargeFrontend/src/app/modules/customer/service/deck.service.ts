...
export class DeckService {
  ...
  createDeck(deckDto: any): Observable<any> {
    console.log(deckDto);
    return this.http.post(`${BASIC_URL}api/customer/deck`, deckDto, {
      headers: this.createAuthorizationHeader(),
    })
  }

  getAllDecks(): Observable<any> {
    return this.http.get<[]>(`${BASIC_URL}api/customer/decks`, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
  ...
  getAllDecksByUserId(userId: any): Observable<any> {
    return this.http.get<[]>(`${BASIC_URL}api/customer/decks/${userId}`, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
      tap((_) => this.log('Decks fetched successfully')),
      catchError(this.handleError<[]>('Error getting decks', []))
  ...
  getDeckById(deckId: any): Observable<any> {
    return this.http.get<[]>(`${BASIC_URL}api/customer/deck/${deckId}`, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
  ...
  getCardsByDeckId(deckId: any): Observable<any> {
    return this.http.get<[]>(`${BASIC_URL}api/customer/cards/${deckId}`, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
      tap((_) => this.log('Deck Fetched successfully')),
      catchError(this.handleError<[]>('Error Getting deck', []))
  ...
  updateDeck(deck: Deck): Observable<Deck> {
    return this.http.put<Deck>(`${BASIC_URL}api/customer/deck/${deck.id}`, deck, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
  ...
  deleteDeckById(deckId: any): Observable<any> {
    return this.http.delete<[]>(`${BASIC_URL}api/customer/deck/${deckId}`, {
      headers: this.createAuthorizationHeader(),
    }).pipe(
